...
| `SENDGRID_API_KEY` | No | — | API key de SendGrid. Si queda vacío, los correos se simulan en consola. |
| `SENDGRID_FROM` | No | — | Dirección "De" verificada en SendGrid. |
| `SENDGRID_FROM_NAME` | No | `Heno Motita` | Nombre visible del remitente. |
...
> **Correo (SendGrid):** el código de activación de cuentas se envía por correo con `internal/services/email_service.go` usando la API v3 de SendGrid (`github.com/sendgrid/sendgrid-go`). En producción (Render) no existe `.env`, por lo que las variables `SENDGRID_*` deben definirse en el panel **Environment** del servicio. Si `SENDGRID_API_KEY` está vacío, el correo no se envía y su contenido se imprime en la consola (modo desarrollo). La dirección de `SENDGRID_FROM` debe estar verificada en el panel de Single Sender Verification de SendGrid.
...
>
> Además de devolverse en la respuesta, el código de activación se envía por correo (API de SendGrid) al alumno. Esto ocurre en el registro masivo (`BatchCreate`), al generar un nuevo código (`new-activation-code`) y al reactivar a un alumno en otra cuadrilla.
